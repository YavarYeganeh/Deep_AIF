...
warm_up_days = 10 #10 #0.33 #10 # days # The warm-up period is imposed for each experiment, which is equal to 10 days of production period
...
# ALPHA = 0.97 # shouldn't be 1 or 0 as those need some adjustments in the code 
...
test_performance_days = 30
test_performance_time_span = test_performance_days * 24 * 60 * 60  # Simulation time in seconds
...
class Prod_System:  # This is the environment with whom the agent interacts
    ...
    def __init__(self, env, dmodel, cap, num_ws, num_m, pw_busy, pw_idle, pw_startup, pw_standby, mttf, mttr, st, pt, reward_multiplier=14, alpha=0.97, policy="Always_ON"):
    ...
    def reward_opf(self):
        ...
        if len(self.consumption_tot_buffer)>1: # avoiding empty buffer
            ...
            r_energy = np.clip(r_energy, 0.0, 1.0)                    # keep it in [0, 1] for numerical stability
        
        else: 
            r_energy = 0
            
        composite_reward = self.alpha * r_prod + (1 -  self.alpha) * r_energy
    ...
    def reward(self):
        ...
        npr
            ...
            New reward/preference function that differentiae energy saving from all-on policy.
          Also consumption rate and throughput of a specified past period compared to the max theoretical ones. This specific function is linear response to throughput and average consumption"""    
        ...
        composite_reward = r_prod * (1 / (1 + np.exp(-self.reward_multiplier * r_energy)))
    ...
    def raw_performance_test(self):
        
        start_parts_made, start_timestamp_parts_made = self.parts_made_start_test
        timespan_parts_made = self.env.now - start_timestamp_parts_made
        if timespan_parts_made < (0.9*test_performance_time_span):
            print ('timespan_parts_made < (0.9*test_performance_time_span) as timespan_parts_made is', timespan_parts_made/(24*60*60), 'days')
        span_parts_made = self.parts_made - start_parts_made
        throughput = span_parts_made / (timespan_parts_made / 60)  # throughput is parts/min

        # consumption 
        start_consumption_tot, start_timestamp_consumption_tot = self.consumption_tot_start_test
        timespan_consumption_tot = self.env.now - start_timestamp_consumption_tot
        if timespan_consumption_tot < (0.9*test_performance_time_span):
            print('timespan_consumption_tot < (0.9*test_performance_time_span) as timespan_consumption_tot is', timespan_consumption_tot/(24*60*60), 'days')
        span_consumption_tot = self.consumption_tot - start_consumption_tot
        avg_consumption = span_consumption_tot / timespan_consumption_tot
        
        #consumption_per_part
        avg_energy_consumption_per_part = span_consumption_tot / span_parts_made

        return throughput, avg_consumption, avg_energy_consumption_per_part
    
    def performance_test(self, reference_throughput=3.0031190052525085, reference_consumption=79.3809907129650014,  reference_energy_consumption_per_part=1586.6565164873900358):
        # Calculate metrics
        throughput, avg_consumption, avg_energy_consumption_per_part = self.raw_performance_test()
    ...
    def start_test_stats(self):
        """ creates reference for beginning of the test  """

        self.parts_made_start_test = self.start_timestamp_now(self.parts_made_buffer)
        self.consumption_tot_start_test = self.start_timestamp_now(self.consumption_tot_buffer)


    # Optimized start_timestamp function using binary search (efficient and only working as the list of keys of the buffer, i.e. times, are sorted!)
    def start_timestamp_now(self, data_buffer):

        target_timestamp = self.env.now
        keys = list(data_buffer.keys())
        lo = 0
        hi = len(keys) - 1
    ...
    # Optimized observation method
    def observation(self):
        o = np.zeros(self.o_dim, dtype=np.float32)
        o[-3:] = self.reward()
        o[self.workstations[0].buffer] = 1.0
        for i in range(self.num_m[0]):
            state = self.workstations[0].machines[i].state -1
            idx = self.bufcap[0] + 1 + i * 5 + state 
            o[idx] = 1.0
        return o
...
class System:
    ...
    def __init__(self, env, number_of_systems=1, dmodel=None, warmup=True, warmup_time=warm_up_time, reward_multiplier=14, alpha=0.97):

        self.env = env
        
        self.systems = [Prod_System(self.env, dmodel=dmodel, cap=BCAP, num_ws=NUM_WORKSTATION, num_m=NUM_MACHINES, pw_busy=W_BUSY, pw_idle=W_IDLE, pw_startup=W_STARTUP, pw_standby=W_STANDBY, mttf=MTTF, mttr=MTTR, st=ST_MEAN, pt=PT_MEAN, policy="Always_ON", reward_multiplier=reward_multiplier, alpha=alpha) for i in range(number_of_systems)]
