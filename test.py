...
- improved preference function
- avoiding span_parts_made == 0 in the raw_performance()
- testing multiple environments
points:
- this version is working with testing multiple environments
- test_model's train and test adjustments 
- lr_test_policy 
 """
...
if npf:
    from src.prod_environment_test_npf_optimized import System
...
sys.stdout = u.Logger(f'{test_results_path}/test_output.txt')
...
for replication in range(test_replications):
    ...
    system_test = System(env=env_test, number_of_systems=1, dmodel=test_model, warmup=False, reward_multiplier=reward_multiplier)
    ...
    while env_test.now < (current_time + response_time):

        o0, ref, oh, r = u.batch_observe(env_test, test_model, sim_interval, batch_size=1, steps=horizon) # simulating h steps

        # planning
        efe, efe_terms = test_model.efe(oh, calc_mean=calc_mean, samples=samples)
        train_policy(test_model, optimizer_test_policy, efe)
    ...
    print_rem_after = current_time + print_rem_interval
    ...
    while env_test.now < (current_time + test_time):
        ...
        # in the interest of memory clearing unnecessary items (o_t/a_t is being used in the batch observer)
        test_model.o_i[0] = []  # Clear observations total lists
        test_model.a_i[0] = []  # Clear action total lists

        if env_test.now > print_rem_after:
            print(f"Replication {replication}: {int((current_time + test_time - env_test.now)/(24*60*60))} simulation days of testing is remaining!")
            print_rem_after += print_rem_interval 
